...
7. 점프 애니매이션
    ...
    4. 점프의 경우 착지를 위해 점프 중임을 체크할 플래그가 필요, bool 파라미터를 추가
...
19. 발사 구현
    ...
    6. 총알이 발사되는 위치 지정
    7. 탄피가 배출되는 위치 지정
    8. 총알 발사 함수를 코루틴을 만든다.
        총알과 탄피를 인스턴스화 하고 지정한 위치에서 생성되게 한다.
        총알은 날아가는 속도를 지정해 주고
        탄피는 떨어지는 힘과 회전 힘을 부여해 준다.
*/
...
#. 카메라 만들기
    ...
    1. 기존 Main Camera는 인게임 카메라로 사용하고 타이틀용 카메라를 만든다.
        기존 카메라를 복사 한다.
        타이틀용 카메라의 위치와 보여줄 UI를 활성화 시킨다.
    2. 화면에 애니매이션을 준다.
        회전값을 주어 조금씩 회전하도록 한다.
*/

/*
36. 최고 점수 기록

    1. PlayerPrefs를 통해 최고 점수를 저장하도록 한다.
*/
...
37. UI 변수 세팅
    ...
    1. 게임 매니저를 만든다.
    2. 카메라를 public으로 받는다.
    3. 플레이어와 보스 스크립트를 public으로 받는다.
    4. 스테이지, 플레이 타임, 전투 중, 몬스터 타입별 갯수를 public으로 만든다.
    5. UI 메뉴 판과 UI 게임 판을 public으로 받는다.
    6. 점수를 출력할 Text들, 스테이지, 플레이 타임 텍스트, 플레이어 체력, 총알, 돈 텍스트를 public으로 받느다.
    7. 장비 이미지 1,2,3,r을 이미지로 각각 public으로 받는다.
    8. 적이 남은 수를 표시할 Text도 각각 public으로 받는다.
    9. 보스 UI의 위치, 에너지 바의 위치
*/

/*
38. 게임 시작

    1. PlayerPrefs에서 저장된 점수를 받아와서 최대 점수 텍스트로 출력한다.
    2. 시작 버튼이 호출할 함수를 만든다.
        메뉴 카메라를 비활성화 하고 게임 카메라를 활성화 한다.
        UI 판도 활성화 비활성화 한다.
        플레이어도 활성화 한다.
*/
...
39. 인게임 UI
    ...
    1. LateUpdate()에서 UI 정보를 출력하도록 한다.
    2. 점수 부터 출력하는데 현재 플레이어 속성에 점수 속성이 없으므로 추가해 준다.
    3. 플레이어 체력, 돈, 총알을 텍스트로 출력한다.
        무기는 장착하고 있는 무기에 따라 0개는 -로 표기하도록 한다.
            이를 위해 Player 스크립트에서 속성으로 지정한 장착 무기를 public으로 변환해준다.
            플레이어가 현재 쥐고 있는 무기에 따라 각기 다른 총알 갯수를 출력한다.
    4. 플레이어 스크립트로 부터 무기 보유 여부를 확인하여 무기 이미지의 알파 값을 조절한다.
    5. 현재 남아 있는 적의 갯수를 출력 한다.
    6. 현재 스테이지를 출력한다.
    7. 현재 플레이 타임을 출력한다.
        플레이 타임은 전투 중인 상태일 때 매 프레임마다 시간을 연산하도록 한다.
    8. 보스의 체력바를 현재 보스의 남은 체력에 비례해서 x 축으로 local스케일을 줄인다.
*/
...
40. 스테이지 관리
    ...
    1. 스테이지 시작, 종료 함수 생성
    2. 스테이지 시작은 트리거 오브젝트를 만들어 상점이나 무기처럼 상호작용을 통해 시작하도록 한다.
        3D Text를 통해 다음 스테이지와 상호작용이란 것을 알려 준다.
    3. 스테이지 존 스크립트를 만든다.
        게임 매니저를 받아와서, 플레이어의 충돌을 감지할 경우 매니저의 스테이지 시작 함수를 호출한다.
    4. 게임 매니저에서 전투 함수를 코루틴으로 만들고 스테이지 시작 함수에서 호출하도록 한다.
        미리 만들어둔 전투 중 플래그에 true
        스테이지가 종료될 때는 플래그에 false
    5. 스테이지가 시작되면 스테이지 존은 사라져야 한다.
    6. 쇼핑 타임도 끝났기 때문에 상점도 모두 사라져야 한다.
    7. 필요 속성 : 아이템샵, 무기샵, 스테이지 존을 담을 게임 오브젝트
    8. 스테이지가 시작될 때 상점과 스테이지를 비활성화 반대는 활성화, 스테이지가 끝날 때 스테이지 번호 증가
    9. 플레이어의 위치를 스테이지가 끝날 때 초기화 한다.
*/

/*
41. 몬스터 관리

    1. 적 스크립트에서 점수와 동전 프리팹을 만든다.
    2. 적이 죽을 때 플레이어에게 점수를 주고 동전을 떨군다.
        플레이어 스크립트를 받아와서 저장한다.
        플레이어 스크립트에 접근하여 자신의 점수를 더한다.
        랜덤한 값을 받는다.
        랜덤값을 인덱스로 활용하여 동전을 인스턴스화 한다.
    3. 몬스터가 스폰되는 위치를 파티클로 만들어서 씬에 배치한다.
    4. 필요 속성 : 몬스터 스폰 위치 배열, 적 프리팹 배열, 몬스터 리스트
    5. 몬스터 리스트에 스테이지에 등장할 몬스터 데이터를 저장한다.
        전투가 시작될 때 현재 스태이지 값 만큼 반복하며 랜덤한 적 타입으로 리스트에 저장한다.
        리스트에 저장된 값 만큼 반복하며 랜덤한 위치에 적을 소환 한다.
        적 스크립트를 받아와서 적의 타겟(플레이어 위치)을 넘겨 준다.
    6. 적의 타입 별로 생성될 때마다 적의 수를 증가 시킨다.
    7. 그리고 5의 배수 스테이지 마다 보스를 소환하도록 한다.
    8. 적의 수가 모두 줄으면 해당 스테이지를 종료 한다.
    9. 적의 수가 줄으는 것은 적 스크립트에서 작성한다.
    10. 보스가 소환될 때만 보스 UI를 띄운다.
*/
...
42. 게임 오버
    ...
    1. 게임 오버 함수를 만든다.
    2. 플레이에게 자신이 죽엇다는 플래그를 만들고 체력이 0 이하로 떨어지면 죽는 애니매이션을 작동한다.
        그리고 플래그 true
        그리고 게임 매니저의 게임 오버 함수를 호출
    3. 게임 씬에 타일틀로 돌아가는 UI를 구성한다.
    4. 게임 매니저에서 UI를 보여줄 수 있도록 변수로 UI를 받는다.
    5. 게임 오버가 되면 기존 UI를 비활성화 하고 게임 오버를 활성화 한다.
        현재 저장되어 있는 최고 점수와 현재 점수를 비교하여 최고 점수이면 저장한다.
    6. 재시작 버튼이 호출할 함수를 만든다.
        씬를 로드한다.
*/
